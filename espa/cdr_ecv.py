...
def makeBrowse(work_dir,metadata, scene_name,resolution=50,debug=False):
    ...
    try:
        ...
        cmds.append(('gdal_translate -ot Byte -scale 0 10000 0 255 -of GTIFF %s/out.tiff5 %s/browse.tiff5') \
            % (extrasdir, extrasdir))
        cmds.append(('gdal_translate -ot Byte -scale 0 10000 0 255 -of GTIFF %s/out.tiff4 %s/browse.tiff4') \
            % (extrasdir,extrasdir))
        cmds.append(('gdal_translate -ot Byte -scale 0 10000 0 255 -of GTIFF %s/out.tiff3 %s/browse.tiff3') \
            % (extrasdir,extrasdir))
        ...
        #Run each of the commands we've built up to now        
        for cmd in cmds:
...
def makeSolrIndex(metadata, scene, work_dir, collection_name,debug=False):
    ...
    try:
        ...
        if metadata.has_key('CORNER_UL_LAT_PRODUCT'):
            upper_left_LL = "%s,%s" % (metadata['CORNER_UL_LAT_PRODUCT'],
                                       metadata['CORNER_UL_LON_PRODUCT'])
            
            upper_right_LL = "%s,%s" % (metadata['CORNER_UR_LAT_PRODUCT'],
                                        metadata['CORNER_UR_LON_PRODUCT'])
            
            lower_left_LL = "%s,%s" % (metadata['CORNER_LL_LAT_PRODUCT'],
                                       metadata['CORNER_LL_LON_PRODUCT'])
            
            lower_right_LL = "%s,%s" % (metadata['CORNER_LR_LAT_PRODUCT'],
                                        metadata['CORNER_LR_LON_PRODUCT'])
            
        else:
            upper_left_LL = "%s,%s" % (metadata['PRODUCT_UL_CORNER_LAT'],
                                       metadata['PRODUCT_UL_CORNER_LON'])
            
            upper_right_LL = "%s,%s" % (metadata['PRODUCT_UR_CORNER_LAT'],
                                        metadata['PRODUCT_UR_CORNER_LON'])
            
            lower_left_LL = "%s,%s" % (metadata['PRODUCT_LL_CORNER_LAT'],
                                       metadata['PRODUCT_LL_CORNER_LON'])
            
            lower_right_LL = "%s,%s" % (metadata['PRODUCT_LR_CORNER_LAT'],
...
def warp_outputs(workdir, projection=None, image_extents=None, pixel_size=None, pixel_unit=None, resample_method=None):
    ...
    def parse_hdf_subdatasets(hdf_file):
        ...
        status,output = commands.getstatusoutput("gdalinfo %s" % hdf_file)
...
def package_product(product_dir, output_dir, product_filename):
    ...
    util.log("CDR_ECV", "Packaging completed product to %s.tar.gz" \
             % (product_file_full_path))
    
    
    #capture whatever the cwd is 
    orig_cwd = os.getcwd()
    
    #move into the product dir so we can tar everything up
    ...
    if status >> 8 != 0:
        util.log("CDR_ECV", "Error packaging finished product to %s.tar" \
                 % (product_file_full_path))
    ...
    #move into the output directory now so we can act on the tar file
    os.chdir(output_dir)
    ...
    if status >> 8 != 0:
        util.log("CDR_ECV", "Error compressing final product file:%s" \
                 % (product_file_full_path))
    ...
    #####################################################################################
    util.log("CDR_ECV", "Changing file permissions on %s to 0644" \
             % (product_file_full_path))
    ...
    #this assumes that we are in the output directory.
    ...
    if status != 0 >> 8:
        ...
        util.log("CDR_ECV", "Couldn't generate cksum against:%s" \
                 % (product_file_full_path))
        ...
        cksum_val = str("%s %s %s") % (str(output[0]),
                                       str(output[1]),
                                       str(cksum_prod_filename))
    
        util.log("CDR_ECV", "Generating cksum:%s" % cksum_val)
        
        #write the cksum file in the output directory
...
def ftp_from_online_cache(username, pw, host, remotefile, localfile):
    """
    Author: David Hill
    ...
    Description:
    Transfers files from the online cache to the local machine
    ...
    util.log("CDR_ECV", "Transfer complete")
       
    
#########################################################################################


def ftp_to_online_cache(username, pw, localfile, host, remotefile):
    """
    Author: David Hill
    ...
    Description:
    Transfers files from the local machine to the online cache
...
def distribute_product(product_file_full_path,
                       cksum_file_full_path,
                       destination_host,
                       destination_file,
    ...
    ssh_destination_host = destination_host
    if not ssh_destination_host.startswith(username):
        ssh_destination_host = "%s@%s" % (username, destination_host)
        
    cmd = "ssh -o StrictHostKeyChecking=no %s mkdir -p %s" \
          % (ssh_destination_host, destination_dir)
    
    util.log("CDR_ECV", "Creating destination directories at %s/%s" % (ssh_destination_host, destination_dir))
    
    util.log("CDR_ECV_DEBUG", "mkdirs cmd:%s" % cmd)
    
    status,output = commands.getstatusoutput(cmd)
    if status != 0:
        util.log("CDR_ECV", "Error creating destination directory %s on %s" \
                 % (destination_dir,ssh_destination_host))
        util.log("CDR_ECV", output)
        return (1,)
    ...
    #####################################################################################
    util.log("CDR_ECV", "Transferring %s to %s:%s" \
             % (product_file_full_path, destination_host, destination_file))   
    
    try:
        ftp_to_online_cache(username,
                            pw,
                            product_file_full_path,
                            destination_host,
                            destination_file)
    ...
    #####################################################################################
    util.log("CDR_ECV", "Transferring %s to %s:%s" \
             % (cksum_file_full_path, destination_host, destination_cksum_file))
    ...
    #####################################################################################
    cmd = "ssh -o StrictHostKeyChecking=no -q %s cksum %s" \
          % (ssh_destination_host, destination_file)
    
    status,output = commands.getstatusoutput(cmd)
    
    if status != 0:
        #problem getting checksum
        util.log("CDR_ECV", "Error generating remote checksum for %s:%s... " \
                 % (ssh_destination_host, destination_file))
        util.log("CDR_ECV", output)
...
def do_distribution(outputdir,
    ...
    #Attempt flag to keep track of how many times packaging and distribution
    #has been attempted
    attempt = 0
    
    #status return flag from packaging
    pstatus = None
    
    #Cksum value from local copy of distributed product file
    local_chksum = None
    
    #Full path to where the cksum file should be distributed
    cksum_file_full_path = None
    
    #Full path to where the product should be distributed
    ...
    while (attempt < 3):
        ...
        pstatus,local_chksum,product_file_full_path,cksum_file_full_path = package_product(workdir,
                                                                                           outputdir,
                                                                                           product_filename)         
        util.log("CDR_ECV", "Pstatus:%s" % pstatus)
        util.log("CDR_ECV", "local_chksum:%s" % local_chksum)
        util.log("CDR_ECV", "product_file_full_path:%s" % product_file_full_path)
        util.log("CDR_ECV", "cksum_file_full_path:%s" % cksum_file_full_path)
                
        #If we got a status 0 back then there were no errors
        if pstatus is not None and pstatus >> 8 == 0:
            util.log("CDR_ECV", "Packaging complete...")
            util.log("CDR_ECV", "Product file:%s" % product_file_full_path )
            util.log("CDR_ECV", "Cksum file:%s" % cksum_file_full_path)
            break
        else:
            #If we didn't break, increment the attempt counter and loop/exit
            attempt = attempt + 1

    if pstatus >> 8 != 0:
        util.log("CDR_ECV", "Error packaging %s" % outputdir)
        raise Exception("Error encountered packaging %s" % outputdir)
    
    
    #reuse the attempt counter
    attempt = 0
    
    #Status flag for the distribution status
    dstatus = None
    
    #Remote cksum value
    remote_cksum = None

    #try to distribute 3 times.
    dstatus = None
    while (attempt < 3):
        
        dstatus,remote_chksum = distribute_product(product_file_full_path,
                                                   cksum_file_full_path,
                                                   destination_host,
                                                   destination_file,
                                                   destination_cksum_file,
        ...
        #################################################################################
        #Status of 0 means its good
        #################################################################################
        if dstatus >> 8 == 0:
            break
        else:
            #If we didn't break, increment and loop/break
            attempt = attempt + 1    
                   
        if dstatus >> 8 != 0:
            util.log("CDR_ECV", "Error encountered distributing products...")
            msg = "Error enountered distributing products:\nProduct File:%s\nCksum File:%s" % (product_file_full_path, cksum_file_full_path)
            raise Exception(msg)
    
    #####################################################################################
    #return the values for the local and remote checksum so the caller can see if they
    #matched
    #####################################################################################
...
if __name__ == '__main__':
    ...
    parser.add_option("--pixel_size",
                      ...
                      help="Desired pixel size for output products.  \
                           If specified --pixel_unit must also be provided.")

    parser.add_option("--pixel_unit",
                       action="store",
                       dest="pixel_unit",
                       choices=['meters', 'dd'],
                       help="Units the supplied pixel_size is specified in.\
    ...
    parser.add_option("--set_image_extent",
                      ...
                      help="Specify desired output image extents \
                           as minx,miny,maxx,maxy (comma seperated) \
                           Be careful to ensure these coordinates correspond to \
    ...
    parser.add_option("--order",
                      action="store",
                      dest="ordernum",
                      help="Includes this scene as part of an order \
                           (controls where the file is distributed to)")

    parser.add_option("--collection",
                      action="store",
                      dest="collection_name",
                      help="Includes this scene as part of a collection \
                           (controls solr index values)")

    parser.add_option("--source_host",
                      action="store",
                      dest="source_host",
                      default="localhost",
                      help="The host where espa should look for \
                           the scene (--scene) to process")
    
    parser.add_option("--source_username",
                      action="store",
                      dest="source_username",
                      help="The host ftp server username")
    
    parser.add_option("--source_pw",
                      action="store",
                      dest="source_pw",
                      help="The host ftp server password")
    ...
    parser.add_option("--destination_username",
                      action="store",
                      dest="destination_username",
                      help="The destination ftp server username")
    
    parser.add_option("--destination_pw",
                      action="store",
                      dest="destination_pw",
                      help="The destination ftp server password")

    parser.add_option("--destination_directory",
                      action="store",
                      dest="destination_directory",
                      help="Directory on the host where the completed \
                           product file should be distributed to")
    ...
    if options.ordernum is None \
    and options.collection_name is None \
    and options.destination_directory is None:
        
        util.log("CDR_ECV", "\n Either an ordernumber,collection name \
                 or destination directory is required \n")
        parser.print_help()
        sys.exit((cdr_ecv_exit_codes.exit_failure,))
    ...
    if options.solr_flag and options.collection_name is None:
        ...
        util.log("CDR_ECV", "\n A collection name was not provided but is required \
                 when generating a solr index \n")
    ...
    if os.environ.get("ESPA_WORK_DIR") == ".":
        #some of python's libs don't interpret "."
    ...
    #####################################################################################
    #Top level directory on the incoming cache server
    #####################################################################################
    base_source_path = '/data/standard_l1t'
    
    #####################################################################################
    #Top level directory on the distribution server
    #####################################################################################
    ...
    #####################################################################################
    #Four digit year of scene colletion date
    #####################################################################################
    year = util.getYear(scene)
    
    #####################################################################################
    #Julian day of the scene collection date
    #####################################################################################
    doy = util.getDoy(scene)
    
    #####################################################################################
    #where do we get the source scene from
    #####################################################################################
    source_host=options.source_host
    
    #####################################################################################
    #where we put the completed scene
    #####################################################################################
    destination_host=options.destination_host
    
    #####################################################################################
    # used to create the DEM filename and for cleanup
    #####################################################################################
    dem_prefix = 'dem.envi' 
    ...
    #####################################################################################
    # Source directory is where we find scenes at to process
    # See if one was passed in.  If not, default to the landsat online cache
    # structure.
    #####################################################################################
    ...
    product_filename = ("%s-SC%s%s%s%s%s%s") % (product_filename,
                                                ts.year,
    ...
    try:
        if os.path.exists(stagedir):
            ...
            status,output = commands.getstatusoutput("rm -rf %s" % stagedir)
    ...
    try:
        if os.path.exists(workdir):
            ...
            status,output = commands.getstatusoutput("rm -rf %s" % workdir)
    ...
    try:
        ...
        util.log("CDR_ECV", "Error cleaning & creating outputdir:%s... exiting" \
                 % (outputdir))
    ...
    ftp_from_online_cache(options.source_username,
                          options.source_pw,
                          source_host,
                          source_file,
    ...
    if (options.sr_ndvi_flag or options.sr_ndmi_flag
        or options.sr_nbr_flag or options.sr_nbr2_flag
        or options.sr_savi_flag or options.sr_evi_flag
        or options.sr_msavi_flag):
        ...
        cmd = ("cd %s; do_spectral_indices.py %s -i %s") \
        % (workdir, index_string, get_sr_filename(scene))
    ...
    if options.sr_flag and not options.cfmask_flag:
        ...
        cmd = ("cd %s; do_append_cfmask.py --sr_infile %s --cfmask_infile %s" \
               % (workdir, get_sr_filename(scene), get_cfmask_filename(scene)))
        status,output = commands.getstatusoutput(cmd)
        if status != 0:
    ...
    if options.sca_flag:
        ...
        cmd = "cd %s; do_snow_cover.py --metafile %s" \
            ...
            % (workdir, metadata['mtl_file'],
               get_toa_filename(scene),
               get_b6thermal_filename(scene),
               get_sca_filename(scene),
               dem_filename)
        ...
        util.log(
            "CDR_ECV",
            "Running CREATE SCA against %s with metafile %s, toa %s, btemp %s, and dem %s" \
    ...
    util.log("CDR_ECV", "Contents:%s" % os.listdir(workdir))
    ...
    #########################################################################################
    # check what else need to be deleted
    #########################################################################################
    ...
    if options.projection or options.image_extent or options.pixel_size:
        ...
        status, output = warp_outputs(workdir,
                                      options.projection,
                                      options.image_extent,
                                      options.pixel_size,
                                      options.pixel_unit,
    ...
    while (attempt < 3):
        local,remote = do_distribution(outputdir,
                                       product_filename,
                                       destination_host,
                                       destination_file,
                                       destination_cksum_file,
        ...
        if local.split()[0] == remote.split()[0]:
            util.log("CDR_ECV", "Distribution complete for %s:%s" \
                     % (destination_host, destination_file))
            success = True
            break
        
        else:
            util.log("CDR_ECV", "Checksums do not match for %s:%s... retrying" \
                     % (destination_host, destination_file))
    ...
    if not success:
        util.log("CDR_ECV", "Packaging and distribution for %s:%s failed after 3 attempts" \
                 % (destination_host,destination_file))
    ...
    if status != 0:
        ...
        util.log("CDR_ECV",
                 "Error cleaning output:%s work:%s stage:%s directories... exiting" \
                 % (outputdir, workdir, stagedir))
    ...
    #do NOT util.log the next line.
    #It must be printed to stdout as the return value for this script
    #########################################################################################
    print ('espa.result={"destination_file":"%s", "destination_cksum_file":"%s"}' \
           % (destination_file, destination_cksum_file))
