...
#==============================================================
def makeBrowse(work_dir,metadata, scene_name,resolution=100,debug=False):
...
def package_product(product_dir, output_dir, product_filename):
    ...
    cmd = ("gzip %s/%s.tar") % (output_dir,product_filename)
    status, output = commands.getstatusoutput(cmd)
    if status != 0:
    ...
    print ("Changing file permissions on  %s/%s.tar.gz to 0644" % (output_dir,product_filename))
    os.chmod("%s/%s.tar.gz" % (output_dir, product_filename), 0644)
    
    #cmd = "chmod 0644 %s/%s.tar.gz" % (output_dir, product_filename)
    #status,output = commands.getstatusoutput(cmd)
    #if status != 0:
    ...
    cmd = ("tar -tf %s/%s.tar.gz") % (output_dir, product_filename)
    status, output = commands.getstatusoutput(cmd)
    if status != 0:
    ...
    cmd = ("md5sum -b %s/%s.tar.gz")% (output_dir, product_filename)
    status, output = commands.getstatusoutput(cmd)
    if status != 0:
        print ("Couldn't generate md5sum against:%s/%s.tar.gz") (output_dir, product_filename)
        print output
        return (4,)                   
    else:
        cksum = output.split()[0]
        return (0, cksum)  
...
def distribute_product(product_file_full_path, destination_host, destination_file):
    ...
    #CHECK DISTRIBUTED PRODUCT INTEGRITY
    #cmd = "ssh %s tar -tf %s/%s.tar.gz" % (destination_host, destination_dir, product_file)
    #status,output = commands.getstatusoutput(cmd)
    #if status != 0:
        #problem with tarfile on distribution server
        
    #CHECK DISTRIBUTED PRODUCT CHECKSUM
    #cmd = "ssh %s md5sum %s/%s.tar.gz" % (destination_host, destination_dir, product_file)
    #status,output = commands.getstatusoutput(cmd)
    #if status != 0:
        #problem getting checksum
    #else:
        #remote_cksum = status.split()[0]
        #if chsum != remote_cksum:
            #problem with chksums matching
...
if __name__ == '__main__':
    ...
    #MAKE THE PRODUCT
    if options.sr_browse_flag or options.sr_ndvi_flag or options.sr_flag or options.b6thermal_flag or options.toa_flag:
    ...
    #PACKAGE THE PRODUCT 
    status,chksum = package_product(outputdir, product_filename)
    
    status,remote_chksum = distribute_product(os.path.join(outputdir, product_filename), destination_host, destination_file)
