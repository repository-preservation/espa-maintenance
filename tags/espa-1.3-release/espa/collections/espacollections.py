    ...
    def execute(self, context):
        if not context.has_key('error') or not len(context['error']) > 0:
            ...
            try:
                ...
                nir = array(nir, dtype = float)  # change the array data type from integer to float to allow decimals
                red = array(red, dtype = float)

                numerator = subtract(nir, red) 
                denominator = add(nir, red)
                ndvi = divide(numerator,denominator)

                #put this into 10000 range
                ndvi = multiply(ndvi, 10000)
                
                #set all negative values to 0
                putmask(ndvi, ndvi < 0, 0)
                
                #set all values greater than 10000 to 10000
                putmask(ndvi, ndvi > 10000, 10000)
