...
class DataSource(models.Model):
    ...
    MAPPING = (
        ('TM','TMDataSource'),
        ('ETM','ETMDataSource'),
        ('NLAPS', 'NLAPSDataSource'),
        ('GLS', 'GLSDataSource'),
        ('DISTRO', 'DistributionDataSource')        
    )
   
    def __unicode__(self):
        return self.name
   
    #should be TM, ETM, GLS2010, ESPA_DISTRO, etc.
    name = models.CharField(max_length=255, choices=MAPPING)
...
class Scene(models.Model):
    ...
    #sourceDS = models.ForeignKey(to=DataSource, related_name="sourceds")
    #destDS = models.ForeignKey(to=DataSource, related_name="destds")
...
class LedapsAncillary(models.Model):
    dayAndYear = models.CharField(max_length=7)
    year = models.IntegerField()
    day = models.IntegerField()
    last_updated = models.DateTimeField(auto_now=True)
    
    
    #data must be base64 encoded before storing and decoded when retrieved
    air = models.TextField(blank=True,null=True)
    water = models.TextField(blank=True,null=True)
    pres = models.TextField(blank=True,null=True)
    ozone = models.TextField(blank=True,null=True)
    
    air_filename = models.CharField(max_length=1024,blank=True,null=True)
    water_filename = models.CharField(max_length=1024,blank=True,null=True)
    pres_filename = models.CharField(max_length=1024,blank=True,null=True)
    ozone_filename = models.CharField(max_length=1024,blank=True,null=True)
