...
class SFTPDataSource(DataSource):
    ...
    def getSFTPClient(self):               
        ...
        connected = False
        attempt_count = 1
        self.logger.log("Creating sftp client to %s:%i" % (self.host,int(self.port)))
        
        while connected == False and attempt_count <= 5:
            try:
                transport.connect(username = self.username, password = self.password)
                connected = True
                self.logger.log("SFTP client connected!")
                break
            except SSHException:
                self.logger.log("SFTP client connection failed, retrying in %i seconds..." % (attempt_count * 5))
                time.sleep(attempt_count * 5)
                attempt_count = attempt_count + 1
                continue

        if not connected:
            raise Exception("Could not create an sftp connection to %s:%i after %i attempts" % (self.host,int(self.port),attempt_count))
...
class DistributionDataSource(SFTPDataSource):
    ...
    def getDataSourcePath(self, sceneid):
        ...
        if self.context.has_key('create_as_collection') and \
            ...
            data_path = ("/data2/LSRD/collections/%s/%s/%s/%s/%s/%s") % (collection,chain,sensor,path,row,year)
        else:
            ...
            data_path = ("/data2/LSRD/orders/%s/%s") % (self.context['order.id'],chain)
    ...
    def execute(self, context):
        ...
        if not context.has_key('error') or not len(context['error']) > 0:
            ...
            try:           
                ...
                ledaps_home = None
                if os.environ.has_key('LEDAPS_HOME'):
                    ledaps_home = os.environ['LEDAPS_HOME']
                else:
                    ledaps_home = '/usr/local/ledaps'

                self.logger.log("Using %s as ledaps_home..." % ledaps_home)
                ledaps_bin = ledaps_home + '/bin'

                if not os.environ.has_key('ANC_PATH'):
                    anc_path = os.path.join(ledaps_home, 'ANC')
                    self.logger.log("ANC_PATH environment variable was not found, defaulting to %s" % anc_path)
                    if os.path.exists(anc_path) and os.path.isdir(anc_path):
                        os.environ['ANC_PATH'] = anc_path
                    else:
                        self.logger.log("%s does not exist, cannot continue" % anc_path)
                    
                os.environ['PATH'] = os.environ['PATH'] + os.pathsep + ledaps_bin

                #make sure we add this to the python path so we can find the class & module
                sys.path.append(ledaps_bin)
                try:
                    from do_ledaps import Ledaps
                except:
                    self.logger.log("Could not import the do_ledaps module!")
                    context['error'] = "LEDAPS: Could not import the do_ledaps module"
                    context['scene.status'] = 'Error'
                    return "ERROR"
                ...
                try:
                    os.chdir(context['work.dir'])
                    self.logger.log("MTL file:%s" % mtl_file)
                    retval = l.runLedaps(metafile=mtl_file, usebin=False)
                except:
                    self.logger.log("Error running LEDAPS")
                    context['error'] = "LEDAPS error detected, terminating"
                    return "ERROR"
                finally:
                ...
                if retval == 0:
                    ...
                    self.logger.log("LEDAPS error detected, terminating")
                    context['error'] = "LEDAPS error detected, terminating"
                    context['scene.status'] = 'Error'
                    return "ERROR"
                else:
                    self.logger.log("Unknown status received from LEDAPS")
                    context['error'] = "Unknown status received from LEDAPS"
                    return "ERROR"
            ...
            try:
                if os.path.isfile(file_path):
                    if (filename.endswith('TIF')
                        ...
                        or filename.startswith('b6clouds')
                        or filename.find('old') != -1
...
if __name__ == '__main__':
    ...
    for line in sys.stdin:
        ...
        context['ledaps.bin.path'] = '/usr/local/ledaps/bin'        
