...
class DistributionDataSource(SFTPDataSource):
    ...
    def getDataSourcePath(self, sceneid):
        ...
        if self.context.has_key('create_as_collection') and \
            ...
            #links if we do that
            data_path = ("/data2/LSRD/orders/%s/%s/%s/%s/%s") % (chain,sensor,path,row,year)
...
if __name__ == '__main__':
    ...
    for line in sys.stdin:
        ...
        #ds = GLSDataSource(logger, context, 'espa', '', 'edcsns7.cr.usgs.gov', 22)
        ds = LandsatDataSource(logger, context, 'espa', 'm1crOM@CHin3s', 'edclpdsftp.cr.usgs.gov', 22)
        ...
        #dds = DistributionDataSource(logger, context, 'espa', '', 'edclxs70.cr.usgs.gov', 22)
        dds = DistributionDataSource(logger, context, 'espa', 'm1crOM@CHin3s', 'edclpdsftp.cr.usgs.gov', 22)
        
        #gsds = GeoSolrDataSource(logger, context, 'espa', '', 'l8srlscp03.cr.usgs.gov', 22)
        gsds = GeoSolrDataSource(logger, context, 'espa', 'm1crOM@CHin3s', 'l8srlscp03.cr.usgs.gov', 22)
