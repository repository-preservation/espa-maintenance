...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Usage: runGLSJob.py inputfile collection_name")
    else:
        ...
        for s in scenes:
            ...
            filler = ""
            entry_length = len(sceneid.strip()) + len(collection.strip()) + 3
            for i in range(1, 512 - entry_length):
                filler = filler + "#"                    

            f.write(sceneid.strip() + '\t' + collection.strip()  + '\t' + filler +'\n')
        f.close()
                                    
        hdfs_target = 'collectionrequests/' + ordername
