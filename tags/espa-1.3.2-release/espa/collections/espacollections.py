    ...
    def execute(self, context):
        if not context.has_key('error') or not len(context['error']) > 0:
            ...
            try:
                ...
                cmds.append(('gdalwarp -dstalpha -srcnodata 0 %s/final.tif %s/warped.tif') % (browsedir, browsedir))
...
if __name__ == '__main__':
    ...
    for line in sys.stdin:
        ...
        #ds = GLSDataSource(logger, context, 'espa', '7iu78iu7*IU&*IU', 'edcsns7.cr.usgs.gov', 22)
        ds = LandsatDataSource(logger, context, 'espa', '7iu78iu7*IU&*IU', 'edclxs140.cr.usgs.gov', 22)
        #dds = DistributionDataSource(logger, context, 'espa', '', 'edclxs70.cr.usgs.gov', 22)
        dds = DistributionDataSource(logger, context, 'espa', '7iu78iu7*IU&*IU', 'edclxs70.cr.usgs.gov', 22)
        
        gsds = GeoSolrDataSource(logger, context, 'espa', 'Esp@_vis', 'l8srlscp03.cr.usgs.gov', 22)
