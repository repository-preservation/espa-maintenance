...
def runCollection(inputfile):
    ...
    input_file_handle = open(inputfile, 'rb+')
    ...
    for i in inputs:
        ...
        filler = ""
        entry_length = len(i.strip()) + 1 
        for x in range(1, 512 - entry_length):
           filler = filler + "#"
                        
        f.write(i.strip() +'\t' + filler +'\n')
    f.close()
...
def usage():
    print 'espa-cron.py run-collection inputfile'
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        ...
        op = sys.argv[1]
        if op == 'run-collection':
            runCollection(sys.argv[2])
