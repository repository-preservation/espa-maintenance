...
##########################################################################################
#  001          01-09-2013      Adam Dosch          Initial release, quick-and-dirty POS.
    ...
    if len(sys.argv) -1 == 1:
        ...
        if len(rows) > 0:
            ...
            try:
                ...
                while not answer:
                    answer = raw_input(" What order(s) do you want to get production info from (e.g. 1,2,3,n): ")
            ...
            for order in productrows.split(","):

                # Get product information for order
                try:
                    cursor.execute("select product_options from ordering_order where id = '%s'" % rows[int(order)][0])
                    
                    prodinfo = cursor.fetchall()
                except Exception, e:
                    db_conn.rollback()
                    print "Exception trying to get production_options from ordering_order for id %s: %s" % (rows[int(order)][0], e)
               
                # Lets un-serialize this json string and load it into python dict data structure
                prod_dict = json.loads(prodinfo[0][0])
                
                # Print out the options that aren't "None" or "False"
                
                print
                print " ========================================="
                print " %s" % rows[int(order)][1]
                print " ========================================="
